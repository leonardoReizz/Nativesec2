  ...
  async execute(organization: ICreateOrganizationRequestDTO) {
    ...
    if (apiCreate.status !== 200 && apiCreate.data.msg !== 'org created') {
      throw new Error(
        ...
        }: Error api create organization, ${JSON.stringify(apiCreate)}`
      );
    }

    const organizationCreated = <OrganizationModelAPI>apiCreate.data.detail[0];
    await Promise.all(
    ...
    if (
      ...
      throw new Error(
        `${
          (store.get('user') as any)?.email
        }: Error api get organization, ${JSON.stringify(getOrganization)}`
      );
    }
